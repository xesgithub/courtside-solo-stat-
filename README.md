...
- **จดสถิติ Live** — 2PT / 3PT (เข้า-พลาด), REB, AST, STL, BLK, TO, PF และปรับแต้มทีมแบบเร็ว
- **นาฬิกาแข่งขัน** — เดินถอยหลังจริงตาม timestamp (กัน drift), แก้เวลา/ควอเตอร์ได้, รองรับต่อเวลา (EX1, EX2, …)
- **Undo / ลบ event** — ลบเหตุการณ์ออกจาก log แล้ว Box Score/แต้มคำนวณใหม่ทันที
- **แก้ทีม/ผู้เล่น** — เพิ่ม/ลบ/แก้ชื่อ-เบอร์, สลับลำดับผู้เล่น
- **Box Score** — ตารางสถิติเต็ม พร้อมแชร์เป็นรูปภาพ (html2canvas)
...
- **Export / Import** — สำรองข้อมูลหรือย้ายเครื่องด้วยไฟล์ JSON (merge อัตโนมัติ ไม่ทับข้อมูลที่ใหม่กว่า)
...
2. **Live** — กดปุ่มสถิติเพื่อบันทึกให้ผู้เล่นแต่ละคน, คุมนาฬิกา (Start/Pause), ปรับคะแนนคู่แข่ง
...
6. **Export / Import** — สำรองข้อมูลเป็นไฟล์ JSON หรือย้ายไปเครื่องอื่น
